...
export function CaseEvidenceTab() {
  ...
  const fetchEvidence = React.useCallback(async () => {
    ...
    const evidenceWithUrls = await Promise.all(
      data.map(async (item) => {
        ...
        if (urlError) {
          console.error("Signed URL hiba:", urlError.message);
  ...
  const handleUpload = async () => {
    ...
    const filePath = `${profile.id}/${caseId}/${uniqueFileName}`;
  ...
  const renderContent = () => {
    ...
    return (
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
        {evidence.map((item) => {
          return (
            <div key={item.id} className="relative group rounded-lg overflow-hidden border border-slate-700 aspect-square">
              {item.signedUrl ? (
                <img
                  src={item.signedUrl}
  ...
  return (
    // --- VÁLTOZÁS: Új kétoszlopos elrendezés ---
    // A flex-1 biztosítja, hogy kitöltse a szülő (CaseDetailPage) által adott helyet
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 flex-1">

      {/* 1. OSZLOP: Feltöltés (ragadós) */}
      <div className="lg:col-span-1 lg:sticky lg:top-[5.5rem] space-y-6">
        <Card className="bg-slate-900 border-slate-700 text-white">
          <CardHeader className="p-6"> {/* p-6 hozzáadva a csupasz Card miatt */}
            <CardTitle>Bizonyíték feltöltése</CardTitle>
            <CardDescription>Csak képfájlok (JPG, PNG, GIF) tölthetők fel.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4 px-6 pb-6"> {/* padding hozzáadva */}
            <Input
              ref={fileInputRef}
              type="file"
              accept="image/*"
      ...
      {/* 2. OSZLOP: Feltöltött képek (görgethető) */}
      <div className="lg:col-span-2 space-y-6">
        <Card className="bg-slate-900 border-slate-700 text-white flex-1 flex flex-col min-h-0">
          <CardHeader className="p-6"> {/* p-6 hozzáadva */}
            <CardTitle>Feltöltött Bizonyítékok</CardTitle>
          </CardHeader>
          {/* A CardContent görgetése NEM itt van, hanem a renderContent kezeli */}
          <CardContent className="p-6"> {/* p-6 hozzáadva */}
            {renderContent()}
          </CardContent>
        </Card>
      </div>

      {/* Képnézegető Modal (Változatlan) */}
      <Dialog open={isImageOpen} onOpenChange={setIsImageOpen}>
        <DialogContent className="bg-slate-900 border-slate-700 text-white w-[95vw] max-w-[95vw] h-[95vh] flex flex-col p-6"> {/* p-6 hozzáadva */}
          <DialogHeader>
          ...
          <DialogFooter className="pt-6"> {/* pt-6 hozzáadva */}
            <DialogClose asChild>
